...
${patientName}患者的${planName}的${taskName}已完成，点击https://hospital-test.ybbhealth.com/patientManger/patientList/index可查看计划详情
发送方式：
{   
    "qiYeWxParam": {
        "isAvailable": true,
...
【医生标记】${doctorName}医生给${patientName}，患者进行标记，内容如下:${signContent}可点击进行查看，https://hospital-test.ybbhealth.com/patientManger/control/index
跳转：
...
{短链}
【待办完成】${doctorName}医生已完成${patientName}患者的${itemType}事项，备注内容${remark}请及时查看，https://hospital-test.ybbhealth.com/patientManger/todo/index
管理后台-患者管理-医生标记信息-对应医生
发送方式：
{   
    "qiYeWxParam": {
...
场景：医生分配问诊时间
群消息：
【问诊分配】{医生姓名}医生完成了{患者姓名}患者的{问诊类型}问诊的时间分配，请及时查看{链接}
【问诊分配】${doctorName}医生完成了${patientName}患者的${inquiryType}问诊的时间分配，请及时查看https://hospital-test.ybbhealth.com/consultation/record/record
发送方式：
{   
    "qiYeWxParam": {
        "isAvailable": true,
...
```json
模版id：yy_inquiry_0000005
标题：分配确认
场景：患者确认分配时间
群消息：
【分配确认】{患者姓名}患者确认了{医生姓名}医生分配的{问诊类型}问诊时间，请及时查看{链接}
【分配确认】${patientName}患者确认了${doctorName}医生分配的${inquiryType}问诊时间，请及时查看https://hospital-test.ybbhealth.com/consultation/record/record
发送方式：
{   
    "qiYeWxParam": {
        "isAvailable": true,
...
```json
模版id：yy_inquiry_0000006
标题：请求重新分配
场景：患者请求重新分配
群消息：
【请求重新分配】{患者姓名}患者拒绝了{医生姓名}医生分配的{问诊类型}问诊时间，请求重新分配，请及时查看{链接}
【请求重新分配】${patientName}患者拒绝了${doctorName}医生分配的${inquiryType}问诊时间，请求重新分配，请及时查看https://hospital-test.ybbhealth.com/consultation/record/record
发送方式：
{   
    "qiYeWxParam": {
        "isAvailable": true,
...
====================================2026.3.3=========================================
```json
模版id：yy_communicate_0000001
标题：新增预约沟通
场景：1.医生提交一个预约沟通(白天)
群消息：
【新增预约沟通】{医生姓名}医生为{患者姓名}患者新增了一个{预约开始时间}时间的预约沟通，请及时查看{链接}
【新增预约沟通】${doctorName}医生为${patientName}患者新增了一个${reserveTime}时间的预约沟通，请及时查看https://hospital-test.ybbhealth.com/consultation/record/record
...
```json
模版id：yy_communicate_0000002
标题：新增预约沟通
场景：医生在晚上9点-第二天早上7点之间提交了一个预约沟通
语音消息：
{医生姓名}医生新增了一个{预约开始时间}时间的预约沟通，请及时通知患者
${doctorName}医生新增了一个${reserveTime}时间的预约沟通，请及时通知患者
发送方式：
{   
"voiceCallParam": {
        "isAvailable": true,
        "templateId": "TTS_328615206"
    }
}
...
[
    ...
    {
        "codeDesc": "预约时间",
        "voiceCode": "reserveTime"
    }
]
```

```json
模版id：yy_communicate_0000003
标题：沟通未完成通知
场景：每天早上9点半，存在未完成通知的沟通
群消息：
当前存在 [未通知数量] 条预约沟通未能完成患者通知，请及时处理{链接}
当前存在 ${unNotifyCount} 条预约沟通未能完成患者通知，请及时处理https://hospital-test.ybbhealth.com/consultation/record/record
发送方式：
{   
    "qiYeWxParam": {
        "isAvailable": true,
...
```json
模版id：yy_member_0000003
标题：会员到期提醒
场景：1.套餐配置了到期提醒，且勾选了对应通知渠道，并且到了到倒计天数的12点时，触发提醒
群消息：
【会员到期提醒】您的患者{患者名}购买的{会员名}将于{天数}日后过期，点击查看详情{会员订单后台连接}
【会员到期提醒】您的患者${patientName}购买的${packageNameAndType}将于${days}日后过期，点击查看详情https://hospital-test.ybbhealth.com/membership/order/index
发送方式：
{   
    "qiYeWxParam": {
        "isAvailable": true,
